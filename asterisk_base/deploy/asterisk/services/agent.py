    ...
    def __init__(self, *args, **kwargs):
        ...
        if os.getenv('GRAYLOG_ENABLED') == '1':
            ...
            class GraylogFilter(logging.Filter):
                # Filter class to add docker container name or other fields
                def __init__(self):
                    self.container_name = os.getenv('GRAYLOG_CONTAINER_NAME',
                                                    'no_container_set')
                    self.uid = os.getenv('CLIENT_UID', 's{}'.format(
                                                        str(uuid.getnode())))
            ...
            kwargs = {
                'localname': os.getenv('GRAYLOG_SOURCE'),
                'level_names': True
            }
            graypy_handler = graypy.GELFHandler(
                                        os.getenv('GRAYLOG_HOST'),
                                        os.getenv('GRAYLOG_PORT'), **kwargs)
            logger.addHandler(graypy_handler)
            logger.addFilter(GraylogFilter())
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(self.odoo_longpoll_url,
                        ...
                        if data.get('error'):
                            # Odoo error
                            logger.error('Odoo error: {}'.format(
                                json.dumps(data['error']['data']['message'],
                                           indent=2)))
                            await asyncio.sleep(
                                float(os.getenv(
